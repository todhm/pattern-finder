...
class FirstCandleRuleSignal:
    ...
    Field semantics:
      ...
      - ``range_break_*`` — the candle that first *closed* above
        ``box_high``. Its low (− 1 tick) is the stop level for
        BOTH paths.
...
class FirstCandleRuleDetector:
    ...
    stop_tick_buffer:
        Subtracted from ``range_break_low`` to set the stop.
        Mimics "stop at the base of the candle that closed
        through". Default $0.01 for US equities.
    ...
    def detect(
        ...
        if self.require_daily_bullish and not df_daily.empty:
            ...
            qualifying = None  # all sessions qualify
    ...
    def _scan_session(
        ...
        def fire_signal(
            ...
            stop = float(range_break_low - self.stop_tick_buffer)
