...
# ========================================
# Dashboard (static page) + Orchestrator endpoints
# ========================================
...
async def orch_list_runs(limit: int = 50):
    from orchestrator import storage as orch_storage
    runs = await orch_storage.list_runs(limit)
    return {"runs": runs, "count": len(runs)}


@app.get("/orchestrator/runs/{run_id}")
async def orch_get_run(run_id: str):
    from orchestrator import storage as orch_storage
    run = await orch_storage.get_run(run_id)
    if not run:
        raise HTTPException(404, detail=f"run {run_id} not found")
    tasks = await orch_storage.list_tasks(run_id)
    return {"run": run, "tasks": tasks}


@app.get("/orchestrator/runs/{run_id}/logs")
async def orch_logs(run_id: str, after_id: int = 0, limit: int = 300):
    from orchestrator import storage as orch_storage
    rows = await orch_storage.tail_logs(run_id, after_id, limit)
    return {"logs": rows, "next_after_id": rows[-1]["id"] if rows else after_id}


@app.post("/orchestrator/runs/{run_id}/resume")
async def orch_resume(run_id: str):
    from orchestrator import runner as orch_runner
    await orch_runner.resume(run_id)
    return {"status": "resumed", "run_id": run_id}


@app.post("/orchestrator/runs/{run_id}/cancel")
async def orch_cancel(run_id: str):
    from orchestrator import runner as orch_runner
    await orch_runner.cancel(run_id)
