...
def retry_on_exception(exceptions, *, max_retries: int = 10, base_delay: float = 15.0, max_delay: float = 300.0):
    ...
    backoff = base_delay * 2**attempt (지수), max_delay 로 cap.

    예시 (base_delay=15):
      attempt 1=15s, 2=30s, 3=60s, 4=120s, 5=240s, 6+=300s
      → 한도 reset (60s) 빠르게 시도하여 throughput 회복,
        반복 fail 시 점진적으로 backoff 늘려 burst detector 회피.
    """
    import functools
...
class AlphaVantageCollector:
    ...
    async def get_pending_symbols(self, csv_file_path: str = None) -> List[str]:
        ...
        try:
            ...
            # Check which symbols have been completed via CollectionLogger (JSON log)
            today = date.today().isoformat()
            completed_set = set()
            ...
            # Also check us_stock_basic table directly — DB 가 source of truth.
            # 오늘 source='api' row 가 이미 있으면 재호출 skip (idempotent re-run 시
            # 같은 날 같은 종목 OVERVIEW 를 다시 호출하지 않음).
            db_today_set = set()
            no_data_set = set()
            try:
                ...
                logger.warning(f"today/no_data lookup failed (continuing): {e}")

            skip_set = completed_set | db_today_set | no_data_set
            pending_symbols = [s for s in all_symbols if s not in skip_set]
        ...
        for i, symbol in enumerate(symbols, 1):
            ...
            if api_data:
                ...
                try:
                    ...
                    logger.warning(f"no_data mark for {symbol} failed: {e_nd}")
                failed_count += 1
...
class DailyCollector:
    def __init__(self, api_key: str, database_url: str, call_interval: float = 0.6,
        ...
        Modes:
        - target_date alone (default behavior, daily-cron):
            collect a single trading day. outputsize=compact (last ~100 days
            from AV, filtered down to that one date).
        - start_date + end_date (historical backfill):
            collect every trading day in [start_date, end_date]. Set
            outputsize='full' (~20yr in one API call/symbol) for full window
            coverage.
        """
        self.api_key = api_key
        # asyncpg postgresql://
        ...
        # Per-symbol override: e.g., {'AAPL': 'compact', 'NEWIPO': 'full'}
        # When symbol not in map, falls back to self.outputsize.
        self.outputsize_per_symbol = outputsize_per_symbol or {}
        # Restrict run to only these symbols (intersection with us_stock_basic).
        # When None, processes all active symbols. Used by task_us_daily to
        # skip symbols that already have complete coverage for the needed range.
    ...
    async def get_existing_symbols(self) -> List[str]:
        ...
        symbols that need updating (e.g. missing dates only).

        Skips symbols marked 'no_data' within last 7 days (excluding today —
        today mark 는 매번 재시도 허용).
        """
        try:
            conn = await self.get_connection()
            rows = await conn.fetch('SELECT symbol FROM us_stock_basic WHERE is_active = true')
    ...
    async def api_worker(self, symbols: List[str], data_queue: Queue):
        """API worker for pipeline pattern - OPTIMIZED with retry for failed symbols"""
        failed_symbols = []  # Store failed symbols for retry
            ...
            try:
                ...
                if api_data:
                    ...
                    if transformed:
                        ...
                        # Transform returned empty - save for retry
                        failed_symbols.append((symbol, eps, bookvalue))
                else:
                    # api_data is None - save for retry
                    logger.warning(f"[US_DAILY] No API data for {symbol} - adding to retry list")
                    failed_symbols.append((symbol, eps, bookvalue))

                if i < len(symbols):
                    await asyncio.sleep(self.call_interval)
            except Exception as e:
                failed_symbols.append((symbol, None, None))
                logger.error(f"[US_DAILY API] Error {symbol}: {e}")

        # Retry failed symbols (up to 2 additional passes)
        for retry_round in range(1, 3):
            if not failed_symbols:
                break

            logger.info(f"[US_DAILY] Retry round {retry_round}: {len(failed_symbols)} symbols to retry")
            await asyncio.sleep(3)  # Wait before retry

            retry_list = failed_symbols.copy()
            failed_symbols = []

            for i, (symbol, eps, bookvalue) in enumerate(retry_list, 1):
                try:
                    date_range_list = self.get_date_range_list()
                    logger.debug(f"[US_DAILY] Retry [{i}/{len(retry_list)}] {symbol}")
                    ...
                    if api_data:
                        ...
                        if transformed:
                            await data_queue.put((symbol, transformed, date_range_list))
                            logger.info(f"[US_DAILY API] Retry success: {symbol}")
                        else:
                            failed_symbols.append((symbol, eps, bookvalue))
                    else:
                        failed_symbols.append((symbol, eps, bookvalue))
                    ...
                    failed_symbols.append((symbol, eps, bookvalue))
                    logger.warning(f"[US_DAILY] Retry failed for {symbol}: {e}")
...
class VWAPCollector:
    ...
    async def api_worker(self, symbols: List[str], data_queue: Queue):
        ...
        for i, symbol in enumerate(symbols, 1):
            try:
                ...
                if api_data:
                    ...
                    # api_data is None - save for retry
                    logger.warning(f"[US_VWAP] No API data for {symbol} - adding to retry list")
                    failed_symbols.append(symbol)
                if i < len(symbols):
                    await asyncio.sleep(self.call_interval)
            except Exception as e:
        ...
        for retry_round in range(1, 3):
            ...
            logger.info(f"[US_VWAP] Retry round {retry_round}: {len(failed_symbols)} symbols to retry")
            await asyncio.sleep(3)  # Wait before retry

            retry_list = failed_symbols.copy()
            failed_symbols = []

            for i, symbol in enumerate(retry_list, 1):
                try:
                    date_range_list = self.get_date_range_list()
                    logger.debug(f"[US_VWAP] Retry [{i}/{len(retry_list)}] {symbol}")
                    ...
                    if api_data:
                        ...
                        if transformed:
                            await data_queue.put((symbol, transformed, date_range_list))
                            logger.info(f"[US_VWAP API] Retry success: {symbol}")
                        else:
                            failed_symbols.append(symbol)
                    else:
                        failed_symbols.append(symbol)
...
class WeeklyCollector:
    ...
    async def get_weekly_data(self, symbol: str, max_retries: int = 3) -> Optional[Dict[str, Any]]:
        """Fetch weekly time series data from Alpha Vantage API.

        rate-limit (`'Note'` or `'Information'` 응답) 시 ``RateLimitError`` 를 raise →
        ``retry_on_exception`` 데코레이터가 60s+ throttle 후 자동 재시도.
        그 외 일시 오류는 함수 내부에서 짧은 backoff 로 재시도.
        """
        await self.init_session()
        ...
        for attempt in range(max_retries):
            try:
                ...
                # 데코레이터로 위임 — 내부 짧은 backoff 가 아니라 60s+ throttle 후 재시도.
                raise
            except asyncio.TimeoutError:
    ...
    async def _mark_no_data(self, symbol: str) -> None:
        """fail 종목을 collection_state.us_weekly 에 mark — 다음 run 에서 skip."""
        try:
            conn = await self.get_connection()
            try:
                await conn.execute(
            ...
            logger.warning(f"[us_weekly] no_data mark for {symbol} failed: {e}")
    ...
    async def collect_symbol_data(self, symbol: str) -> int:
        ...
        try:
            ...
            if api_data:
                ...
                # → no_data mark 로 다음 run 에서 skip (today 는 retry 허용).
                logger.warning(f"Failed to fetch weekly data for {symbol}; marking no_data")
...
class IPOCalendarCollector:
    ...
    async def fetch_ipo_calendar(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"[US_IPO] Error fetching IPO calendar: {e}")
            return []

    async def save_ipo_data(self, data: List[Dict[str, Any]]) -> tuple:
        """Save IPO calendar data to database with upsert"""
        if not data:
            return 0, 0
        ...
        try:
            for record in data:
                ...
                result = await conn.execute("""
                    ...
                    ON CONFLICT (symbol, ipodate) DO UPDATE SET
                        name = EXCLUDED.name,
                        pricerangelow = EXCLUDED.pricerangelow,
                        pricerangehigh = EXCLUDED.pricerangehigh,
            ...
            logger.error(f"[US_IPO] Error saving IPO data: {e}")
            return inserted, updated
        finally:
            await conn.close()

    async def collect_ipo_calendar(self):
        """Main collection function for IPO calendar"""
        logger.info("=== US IPO Calendar Collection Started ===")
        start_time = datetime.now()

        try:
            data = await self.fetch_ipo_calendar()

            if not data:
                logger.warning("[US_IPO] No IPO data fetched")
                return

            inserted, updated = await self.save_ipo_data(data)

            duration = datetime.now() - start_time
            logger.info("=== US IPO Calendar Collection Summary ===")
            logger.info(f"Total execution time: {duration}")
            logger.info(f"Total records from API: {len(data)}")
            logger.info(f"Inserted: {inserted}, Updated: {updated}")
            logger.info("=== US IPO Calendar Collection Completed ===")

        except Exception as e:
            logger.error(f"[US_IPO] Collection failed: {e}")
        finally:
            await self.close_session()


class EarningsHistoryCollector:
    """AV EARNINGS endpoint → us_earnings_history.

    각 종목당 1 call. 응답의 quarterlyEarnings 가 정확한 reportedDate 를 포함.
    ...
    async def save_earnings(self, symbol: str, data: Dict[str, Any]) -> int:
        ...
        async with self.pool.acquire() as conn:
            await conn.executemany(
                """INSERT INTO us_earnings_history
    ...
    async def process_symbol(self, symbol: str) -> int:
        async with self.semaphore:
            try:
                data = await self.fetch_earnings(symbol)
                if not data:
                    await self._mark_no_data(symbol)
                    return 0
                n = await self.save_earnings(symbol, data)
                if n == 0:
                    await self._mark_no_data(symbol)
                await asyncio.sleep(self.call_interval)
                return n
            except Exception as e:
                logger.error(f"[EARNINGS_HIST] {symbol}: {e}")
                return 0
