...
# Reuse the Mongo-backed CSV store from kr/gcs_handler.py
_THIS_DIR = Path(__file__).parent
_KR_DIR = _THIS_DIR.parent / "kr"
if str(_KR_DIR) not in sys.path:
    sys.path.insert(0, str(_KR_DIR))
from gcs_handler import get_gcs_handler  # noqa: E402
...
class StockListingDownloader:
    def __init__(self, database_url: Optional[str] = None):
        ...
        # MongoDB-backed CSV store (was GCS)
        self.csv_store = get_gcs_handler()
    ...
    def upload_to_gcs(self, local_path: Path) -> bool:
        """Upload CSV file to MongoDB CSV store (name kept for backward compat)."""
        try:
            with open(local_path, "r", encoding="utf-8") as f:
                csv_data = f.read()
            ok = self.csv_store.upload_file(local_path.name, csv_data, folder="us")
            if ok:
                logger.info(f"Uploaded {local_path.name} to MongoDB (folder=us)")
            return ok
        except Exception as e:
            logger.error(f"Failed to upload to MongoDB: {str(e)}")
            return False
