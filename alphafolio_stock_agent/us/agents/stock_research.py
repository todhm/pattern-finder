"""
Stock Research Agent module for US stocks
...
3. LLM extracts: earnings_outlook, risk_issues, sector_momentum, insider_activity
...
from us.data.research_collector import (
    collect_research_data,
    determine_stock_size
)
from us.data.research_preprocessor import (
    preprocess_research_data,
    format_for_llm
)
from us.prompts.stock_research import (
    STOCK_RESEARCH_SYSTEM_PROMPT,
    format_stock_research_prompt,
    format_research_retry_prompt,
    validate_research_output
)
from us.utils.debug_saver import save_stock_research
...
async def collect_stock_research_data(
    ...
    """
    Collect all qualitative research data for a US stock.
...
async def collect_stock_research_node(state: AgentState) -> dict:
    ...
    This node:
    1. Gets stock detail info (sector, market_cap) from DB
    2. Collects research data (analyst reports, SEC filings, news, insider)
    ...
    try:
        ...
        # Get stock basic info for sector and market_cap
        ...
        if not stock_basic:
            ...
            sector = stock_basic.get("sector", "Unknown")
            market_cap = float(stock_basic.get("marketcapitalization", 0) or 0)
...
async def analyze_stock_research(state: AgentState) -> dict:
    ...
    2. Calls LLM to extract earnings_outlook, risk_issues, sector_momentum, insider_activity
    ...
    try:
        ...
        # Format text for LLM
        formatted_text = preprocessed.get("formatted_text", "No data available")
        ...
        user_prompt = format_stock_research_prompt(
            ...
            stock_size=stock_info.get("stock_size", "mid"),
            ...
            insider_count=processed_counts.get("insider_transactions", 0),
            filing_count=processed_counts.get("sec_filings", 0)
        )
        ...
        result["metadata"] = {
            "data_collection": {
                "analyst_reports": processed_counts.get("analyst_reports", 0),
                "news_articles": processed_counts.get("stock_news", 0) + processed_counts.get("sector_news", 0),
                "sec_filings": processed_counts.get("sec_filings", 0),
                "insider_transactions": processed_counts.get("insider_transactions", 0)
            },
            "market_cap_tier": stock_info.get("stock_size", "mid"),
...
async def stock_research_node(state: AgentState) -> dict:
    ...
    1. Data collection (analyst reports, SEC filings, news, insider)
    2. Preprocessing
    3. LLM extraction
    4. Validation
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        print(f"Running US Stock Research Analysis for {test_symbol}...")
        print("-" * 50)
